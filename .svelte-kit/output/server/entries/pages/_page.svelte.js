...
function AnimatedHeroText($$renderer, $$props) {
  $$renderer.component(($$renderer2) => {
    ...
    {
      $$renderer2.push(`<h2${attr_class(`absolute text-5xl md:text-7xl font-bold tracking-tight text-center px-4 transition-colors duration-1000 ${stringify(environment.dayCycle === "day" ? "text-slate-900 drop-shadow-sm" : "text-white drop-shadow-2xl")}`)}>${escape_html(messages[currentIndex])}</h2>`);
    }
    $$renderer2.push(`<!----></div> `);
    {
      $$renderer2.push("<!--[!-->");
...
function ExperienceSection($$renderer, $$props) {
  $$renderer.component(($$renderer2) => {
    ...
    $$renderer2.push(`<section class="relative w-full h-[400vh] bg-neutral-950"><div class="sticky top-0 w-full h-screen overflow-hidden flex flex-col justify-center bg-neutral-950 transition-colors duration-1000"><div${attr_class(`absolute inset-x-0 top-1/2 -translate-y-1/2 h-[600px] w-full max-w-4xl mx-auto ${stringify(activeColor)} opacity-10 blur-[150px] transition-colors duration-[1500ms] rounded-full pointer-events-none`)}></div> <div class="absolute top-12 left-8 md:top-24 md:left-24 z-20"><span class="text-white/40 font-bold uppercase tracking-[0.4em] text-[10px] mb-2 block animate-pulse">Evolution</span> <h2 class="text-5xl md:text-7xl font-black text-white mix-blend-exclusion">Trajetória</h2></div> <div class="flex items-center gap-12 md:gap-32 px-[10vw] md:px-[30vw] min-w-max transition-transform duration-75 ease-out relative z-10"${attr_style(`transform: translateX(-${stringify((trackWidth - innerWidth) * globalProgress)}px);`)}><!--[-->`);
...
function ProjectsSection($$renderer) {
  ...
  $$renderer.push(`<section class="py-40 bg-black text-white relative overflow-hidden"><div class="absolute inset-0 bg-[linear-gradient(rgba(255,255,255,0.03)_1px,transparent_1px),linear-gradient(90deg,rgba(255,255,255,0.03)_1px,transparent_1px)] bg-[size:64px_64px] [mask-image:radial-gradient(ellipse_60%_60%_at_50%_50%,#000_10%,transparent_100%)]"></div> <div class="max-w-7xl mx-auto px-8 relative z-10"><div class="mb-24 md:flex items-end justify-between"><div class="max-w-xl"><span class="text-blue-500 font-bold uppercase tracking-[0.4em] text-[10px] mb-4 block">Portfólio</span> <h2 class="text-5xl md:text-7xl font-black tracking-tighter mb-6 text-balance">Nível de <br/><span class="text-transparent bg-clip-text bg-gradient-to-r from-blue-400 to-emerald-400">Produção</span></h2></div> <p class="text-slate-400 max-w-sm font-light leading-relaxed mb-4 md:text-right">Aplicações reais focadas em conversão, tráfego qualificado e experiência estética de ponta.</p></div> <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8"><!--[-->`);
  const each_array = ensure_array_like(projects);
  for (let $$index_1 = 0, $$length = each_array.length; $$index_1 < $$length; $$index_1++) {
    let project = each_array[$$index_1];
    $$renderer.push(`<a${attr("href", project.url)} target="_blank" rel="noopener noreferrer" class="group block p-8 rounded-[2rem] bg-white/[0.02] border border-white/5 hover:border-white/10 hover:bg-white/[0.04] backdrop-blur-md transition-all duration-500 hover:-translate-y-2 relative overflow-hidden"><div${attr_class(`absolute inset-x-0 top-0 h-1 bg-gradient-to-r ${stringify(project.color)} opacity-40 group-hover:opacity-100 transition-opacity duration-500`)}></div> <div class="flex justify-between items-start mb-12"><div class="w-12 h-12 rounded-xl bg-white/5 flex items-center justify-center backdrop-blur-md border border-white/10 group-hover:scale-110 group-hover:bg-white/10 transition-all duration-500"><svg xmlns="http://www.w3.org/2000/svg" width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" class="text-white"><path d="M2 3h6a4 4 0 0 1 4 4v14a3 3 0 0 0-3-3H2z"></path><path d="M22 3h-6a4 4 0 0 0-4 4v14a3 3 0 0 1 3-3h7z"></path></svg></div> <div class="w-10 h-10 rounded-full bg-white/5 flex items-center justify-center group-hover:bg-white group-hover:text-black transition-colors duration-500"><svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" class="group-hover:translate-x-0.5 group-hover:-translate-y-0.5 transition-transform"><line x1="7" y1="17" x2="17" y2="7"></line><polyline points="7 7 17 7 17 17"></polyline></svg></div></div> <h3 class="text-2xl font-bold mb-3 text-white group-hover:text-blue-400 transition-colors">${escape_html(project.name)}</h3> <p class="text-slate-400 text-sm font-light leading-relaxed mb-8 h-20 text-balance">${escape_html(project.desc)}</p> <div class="flex flex-wrap gap-2"><!--[-->`);
    const each_array_1 = ensure_array_like(project.tags);
    for (let $$index = 0, $$length2 = each_array_1.length; $$index < $$length2; $$index++) {
      let tag = each_array_1[$$index];
      $$renderer.push(`<span class="px-3 py-1 rounded-full bg-white/5 border border-white/10 text-[10px] font-medium tracking-wide text-slate-300">${escape_html(tag)}</span>`);
    }
    $$renderer.push(`<!--]--></div></a>`);
  }
