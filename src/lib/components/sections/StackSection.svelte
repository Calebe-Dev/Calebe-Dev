<script lang="ts">
    ...
    const stack = [
        { id: '01', name: 'SvelteKit', icon: '⚡', color: '#ff3e00', desc: 'Performance SSR nativa para interfaces fluidas.', category: 'Frontend', group: 0 },
        { id: '02', name: 'Angular', icon: '🅰️', color: '#dd0031', desc: 'Estruturas de alta complexidade e escalabilidade.', category: 'Frontend', group: 0 },
        { id: '03', name: 'JavaScript', icon: 'JS', color: '#f7df1e', desc: 'Padrões modernos e execução do alto impacto.', category: 'Frontend', group: 0 },
        { id: '04', name: 'PostgreSQL', icon: '🐘', color: '#336791', desc: 'Segurança e integridade na persistência de dados.', category: 'Backend', group: 1 },
        { id: '05', name: 'Ruby', icon: '💎', color: '#701516', desc: 'Elegância e agilidade no desenvolvimento backend.', category: 'Backend', group: 1 },
        { id: '06', name: 'Python', icon: '🐍', color: '#3776ab', desc: 'Processamento eficiente e inteligência aplicada.', category: 'Infrastructure', group: 2 },
        { id: '07', name: 'Go', icon: '🐹', color: '#00add8', desc: 'Concorrência e eficiência em sistemas distribuídos.', category: 'Infrastructure', group: 2 }
    ];
    ...
    function getCardStyle(item: any, prog: number) {
        ...
        const dist = Math.abs(localProgress - 0.5) * 2;
        
        let intensity = Math.max(0, dist - 0.2) * 5; 
        ...
        return `
            transform: translate3d(${(random(seed)-0.5)*100*intensity}px, ${(random(seed+1)-0.5)*100*intensity}px, ${-intensity*1000}px);
            opacity: ${1 - intensity};
...
>
    ...
    <div class="absolute inset-0 bg-[radial-gradient(circle_at:100%_100%,_rgba(59,130,246,0.04),_transparent_70%)] pointer-events-none"></div>
    ...
    <div class="sticky top-0 w-full h-screen flex flex-col items-center justify-center overflow-hidden px-8">
        ...
        <div 
            class="absolute top-20 text-center z-50 transition-all duration-1000 cubic-bezier(0.16, 1, 0.3, 1)"
            style="opacity: {1 - assemblyIntensity}; filter: blur({assemblyIntensity * 15}px); transform: translateY({assemblyIntensity * 30}px);"
        >
            <span class="text-blue-500 font-bold uppercase tracking-[0.8em] text-[10px] mb-6 block opacity-50">Power Grid</span>
            <h2 class="text-fluid-title font-black tracking-tighter leading-none mb-4 uppercase">
                Minha <span class="text-transparent bg-clip-text bg-gradient-to-r from-blue-400 to-indigo-500">Stack</span>
            </h2>
        </div>
        ...
        <div class="perspective-[2500px] w-full max-w-6xl">
            <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-8">
                {#each stack as item}
                    ...
                    >
                        <div class="w-16 h-16 mb-8 rounded-2xl bg-white/5 flex items-center justify-center text-4xl border border-white/5 transition-transform duration-500 group-hover:scale-110">
                            {item.icon}
                        </div>
        ...
        <div class="absolute bottom-12 flex flex-col items-center gap-4 opacity-20">
            <span class="text-[8px] font-black tracking-[0.6em] uppercase tabular-nums">Analysis {Math.floor(progress * 100)}%</span>
